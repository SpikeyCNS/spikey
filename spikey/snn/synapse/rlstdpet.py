...
class RLSTDPET(RLSynapse):
    ...
    def _apply_stdp(self, full_spike_log: np.bool, inhibitories: np.bool):
        ...
        post_dts = np.sum(dts, axis=0)[w_pre_locations]
        ...
        if w_pre_locations.size:
            self.weights._matrix[post_locations, w_pre_locations] -= (
                inhibitories[post_locations] * post_dts * update_mult
            )
