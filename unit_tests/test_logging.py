...
class TestLog(unittest.TestCase, BaseTest):
    ...
    def test_sanitize(self):
        for dictionary in [None, 12.3, (2, 4), {}, {'one': 123, 24: 25}, {'obj': type('Obj', (object,), {})}, {'np.int': np.int_(12)}]:
            output = sanitize(dictionary)
            json_output = json.loads(json.dumps(output))

    def test_serialize(self):
        for shape in [0, 1, 100, (10, 10), (10, 10, 10)]:
            for original in [np.ones(shape, dtype=np.int), np.random.uniform(0, 1, shape), np.zeros(shape, dtype=np.bool)]:
    ...
    def test_usage(self):
        network = type('SpoofNetwork', (object,), {'parts': {}, 'params': {'p1': 1, 'p2': 2.3}})
        ...
        for kwparams in [
            ...
            with self.subTest(f"{kwparams}"):
                ...
                if "network" in kwparams:
                    np.testing.assert_array_equal(list(network.params.values()), reader.network.values[0])
                if "game" in kwparams:
                    np.testing.assert_array_equal(list(game.params.values()), reader.game.values[0])
                if "results" in kwparams:
                    np.testing.assert_array_equal(list(results.values()), reader.results.values[0])
                if "info" in kwparams:
